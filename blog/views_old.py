...
from blog.models import Company, City, Meeting, UserNorm, PhoneCalls
from django.db.models import Q
from blog.forms import PostForm, PlanPhoneCallForm
...
def done_meeting(request):
	if request.method == "GET":
		query = request.GET.get("done")
...
def mystats(request):
	...
	m_planned = m_all.filter(done=0, date__gt=datetime.now())
	m_done = m_all.filter(done=1)

	# Monthly meetings
	mm_done = m_all.filter(done=1, date__month=datetime.now().month)
	mm_norm = UserNorm.objects.get(employee_id=request.user.id).meetings_month_norm
	mm_pct = int(100*len(mm_done)/mm_norm)
	m_to_acc = m_all.filter(done=0, date__lte=datetime.now())
